...
class Settings 
{
    ...
    { 
        ...
        if (!isset($path)) {
            $path = $REX['INCLUDE_PATH'].DIRECTORY_SEPARATOR.'data'.DIRECTORY_SEPARATOR.'addons';
        }
    ...
    public function initSettings(array $presets = array())
    {
        ...
        if (file_exists($filename)) {
            $settings = array_merge($presets, $this->loadSettings());
        } else {
            $settings = $presets;    
        }
     ...
     *
     * @param   $ignore_missing_file  True if the error message shall be supressed
    ...
    public function loadSettings($ignore_missing_file = FALSE)
    {
        ...
        if (file_exists($filename)) {
            ...
            $settings = FALSE;
            if (!$ignore_missing_file && $REX['REDAXO']) {
