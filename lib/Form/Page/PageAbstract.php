...
abstract class PageAbstract
{
    ...
    protected $style = array('style' => 'width:70%;font-size:0.8em;');
    ...
    /**
     * Get data of a table as array(id => name_x)
     */
    ...
    public function getName($table)
    {
        ...
        $sql->setDebug($this->debug);
        $sql->setTable($table);
        $sql->setWhere("`status` = 1 ORDER BY `name` ASC");
        $sql->select("`id`, `name`");
        $rows = $sql->getArray();
        return $rows;
    }
     ...
     * Get a list of names
     */
    public static function getNameList($p)
    {
        $sql = \rex_sql::factory();
